...
class Shape:
    def __init__(
        ...
        if shape_type not in [
            'sphere','polyhedron', 'sphero_cylinder', 'sphero_polygon',
            'sphero_polyhedron', 'briquete', 'faceted_cylinder', 'assembly',
            'straight_fiber', 'custom_fiber', 'custom_shell', 'custom_polyhedron'
        ]:
            raise ValueError(f"Invalid shape type: {shape_type}. Check for typos.")
        else: self.shape_type = shape_type
...
class Polyhedron(Shape):
    def __init__(self, radius: float | dict[float, float],
                 vert_ar: float, horiz_ar: float, n_corners: int, superquadric_degree: float) -> None:
        self.radius = radius
        self.vert_ar = vert_ar
        self.horiz_ar = horiz_ar
        self.n_corners = n_corners
        self.sq_degree = superquadric_degree
        super().__init__(shape_type='polyhedron', radius=radius, vert_ar=vert_ar, horiz_ar=horiz_ar,
                         n_corners=n_corners, sq_degree=superquadric_degree)
...
class CustomPolyhedron(Shape):
    ...
    def particle2rocky(self, particle, material, rolling_friction: float = 0.0):
        particle.ImportFromSTL(
            stl_filename=self.stl_path,
            scale=1.0
        )
        if material:
            particle.SetMaterial(material)
