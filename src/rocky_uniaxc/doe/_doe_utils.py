...
class SimParams:
    ...
    Attributes:
        ...
        fric_rolling: Rolling friction coefficient (particle-particle).
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"    μ_pp={self.fric_dyn_pp:.3g}/{self.fric_stat_pp:.3g}, μ_pw={self.fric_dyn_pw:.3g}/{self.fric_stat_pw:.3g} (μr={self.fric_rolling:.3g}),\n"
    ...
    def __post_init__(self):
        if self.adhesion == "JKR" and self.normal != "damped_hertzian":
            raise ValueError(
                "JKR adhesion model requires Hertz normal force model"
                "- use 'damped_hertzian' for normal contact model"
            )
    ...
    def from_tuple(cls, params: tuple, shape: ShapeConfig | dict) -> SimParams:
        ...
        return cls(
            ...
            poisson=params[2],
            youngmod=params[3],
            fric_rolling=params[4],
            surf_en_pp=params[5],
            fric_dyn_pp=params[6],
            fric_stat_pp=params[7],
            tan_stiff_r_pp=params[8],
            cor_pp=params[9],
            surf_en_pw=params[10],
            fric_dyn_pw=params[11],
            fric_stat_pw=params[12],
            tan_stiff_r_pw=params[13],
            cor_pw=params[14],
            box_len=params[15],
            p_compress=params[16],
            normal=params[17],
            tangential=params[18],
            rolling=params[19],
            adhesion=params[20],
            shape=shape,
        )
...
def script_context_from_params(
    ...
    return {
        ...
        "XPU": target,
        "MESH_DIR": meshdir,
        "ROLLING_FRICTION": params.fric_rolling,
    }
