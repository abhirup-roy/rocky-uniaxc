...
class UniaxialCompression:
    ...
    def __init__(self,**kwargs):
        ...
        ----------
        - project_dir : str
            The working directory for the simulation. Default is the current working directory.
        - radius : float
        ...
        self.particle_box_len:float = kwargs.get('particle_box_len', 10/1000)  # m
    ...
    def _setup(self):
        ...
        self.project.SaveProject(os.path.join(self.project_dir, 'uniaxial_compression.rocky'))
    ...
    def _load_meshes(self):
        ...
        insert_support1 = self.study.ImportWall(support1_stl_path,
                                                import_scale=self.particle_box_len,
                                                convert_yz=False)[0]
        insert_support1.SetName('Insert Support 1')
        insert_support1.SetDisableTime(2,'s')
        insert_support2 = self.study.ImportWall(support2_stl_path,
                                                import_scale=self.particle_box_len,
                                                convert_yz=False)[0]
        insert_support2.SetName('Insert Support 2')
        insert_support2.SetDisableTime(2,'s')
        insert_support3 = self.study.ImportWall(support3_stl_path,
                                                import_scale=self.particle_box_len,
                                                convert_yz=True)[0]
        ...
        insert_support4 = self.study.ImportWall(support4_stl_path,
                                                import_scale=self.particle_box_len,
                                                convert_yz=True)[0]
        ...
        # Insert inlet plane
        self.insert_inlet = self.study.ImportSurface(inlet_stl_path,
                                                import_scale=self.particle_box_len,
                                                convert_yz=False)[0]
    ...
    def _domain_settings(self):
        ...
        self.domain.SetCartesianPeriodicDirections('XY') #no periodic in Z-direction to allow compression
    ...
    def _insertion_settings(self):
        ...
        n_particles = int(fill_box_vol * 0.7 / particle_vol) # 0.7 is a 'max' packing fraction
        mass_particles = particle_vol * self.p_density * n_particles

        particle_inlet = self.study.CreateParticleInlet(self.insert_inlet, self.particle)
